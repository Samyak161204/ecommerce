  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      if (!cartItems || !products) {
        toast.error("Cart is empty or products are unavailable.");
        return;
      }
      ...
      if (orderItems.length === 0) {
        toast.error("No valid items to place an order.");
        return;
      }

      let orderData = {
        userId: token, // Ensure user ID is included
        address: formData,
        items: orderItems,
      ...
      switch (method) {
        case "cod":
          if (!token) {
            toast.error("Authentication error: Token missing.");
            return;
          }
